...
import { createTOTPDevice, getTOTPQRCode, verifyTOTPDevice } from "@/lib/api";
...
export default function Setup2FAPage() {
  ...
  const fetchQRCode = async () => {
    try {
      // getTOTPQRCode should return the raw blob (arraybuffer/blob response type)
      const blob = await getTOTPQRCode({ email });
  ...
  const handleVerify = async (e) => {
    ...
    setError("");
    try {
      await verifyTOTPDevice({ email, otp_token: otpToken });
      setStep(3);
    } catch (err) {
      const res = err?.response?.data;
      setError(
